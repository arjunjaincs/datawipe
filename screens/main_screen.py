...
class WipeThread(QThread):
    ...
    def run(self):
        """Simulate wipe progressing linearly 0 → 100 with steady logs.
        Each percentage takes 15–30 seconds (random) with frequent log updates.
        """
        ...
        # Continuous percent-by-percent progress
        for pct in range(1, 101):
            if not self.is_running:
                return

            # Duration for this percent
            target_ms = random.randint(15000, 30000)
            elapsed = 0

            # Stream logs during this percent
            while elapsed < target_ms and self.is_running:
                action = random.choice([
                    "Overwriting block", "Verifying block", "Checksum", "Queue depth",
                    "DMA throughput", "Entropy pool", "Zeroing padding", "Sync flush",
                    "TRIM command issued", "Secure erase routine", "Pattern verification"
                ])
                ...
                nap = random.randint(150, 400)
                self.msleep(nap)
                elapsed += nap
                if random.random() < 0.08:
                    emit_log("Controller thermal check: nominal • HPA/DCO clear queued")

            # Update percent when this slice is done
